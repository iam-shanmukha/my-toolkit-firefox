(function () {
    if (window.__claudeSelectionPopupLoaded) return;
    window.__claudeSelectionPopupLoaded = true;
    ...
    // Parses fixed, author-authored markup (never user/runtime data) into
    ...
    function getTextFromInput(el) {
        ...
        return selected || el.innerText?.trim() || '';
    }
    ...
    function buildHost() {
        ...
        shadow.appendChild(parseTrustedFragment(`
            <style>
                :host { display: flex; }
                button {
                    all: unset;
    ...
    function onIconClick(provider) {
        const el = activeInput;
        if (!el) return;
        const text = getTextFromInput(el);
        hideIcon();
        if (provider === 'chatgpt') {
            chrome.storage.local.set({ chatgptPendingText: text }, () => {
                chrome.runtime.sendMessage({ action: 'openChatGPT' });
            });
            return;
        }
        chrome.storage.local.set({ claudePendingText: text }, () => {
            chrome.runtime.sendMessage({ action: 'openClaude' });
        });
    }
